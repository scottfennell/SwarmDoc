...
NotoAir.Page = function(configObj){
	...
	this.getHistoryOffset = function(historyOffset){
		...
		if(this.text[historyOffset].text == undefined){
            var histId = this.text[historyOffset].id;
            this.text[historyOffset].text = this.getHistoryId(histId);
        }
		//fix issue if the the text portion is null
		if(this.text[historyOffset].text == null){
			this.text[historyOffset].text = "";
		}
		return this.text[historyOffset].text;		
	}
