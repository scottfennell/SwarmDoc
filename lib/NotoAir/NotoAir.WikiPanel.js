...
NotoAir.WikiPanel = Ext.extend(Ext.Panel, {
	...
	lastHistory : function(e,t){
	    ...
	    if(this.historyOffset<this.currentPage.history.length){
	        //TODO put the text into the page
	        console.log(this.currentPage.history[this.historyOffset]);//0 is most recent
	        if(this.currentPage.history[this.historyOffset].text == undefined){
	            var histId = this.currentPage.history[this.historyOffset].id
	            this.currentPage.history[this.historyOffset].text = NotoAir.DB.getTextFromID(histId);
	        }
	        //currently displayed
	        if(this.currentPage.history[this.historyOffset-1].text == undefined){
	            var currId = this.currentPage.history[this.historyOffset-1].id
	            this.currentPage.history[this.historyOffset-1].text = NotoAir.DB.getTextFromID(currId);
	        }

	        var textCmp = NotoAir.WikiParser.parseText(
	            this.doDiff(this.currentPage.history[this.historyOffset-1].text,
	            this.currentPage.history[this.historyOffset].text)
	        );
	...
	nextHistory : function(e,t){
	    ...
	    if(this.historyOffset >= 0){
	        ...
	        if(this.currentPage.history[this.historyOffset+1].text == undefined){
	            var histId = this.currentPage.history[this.historyOffset+1].id
	            this.currentPage.history[this.historyOffset+1].text = NotoAir.DB.getTextFromID(histId);
	        }
	        var textCmp = NotoAir.WikiParser.parseText(
	            this.doDiff(this.currentPage.history[this.historyOffset+1].text,//startText
	            this.currentPage.history[this.historyOffset].text)//endText
	        );
	        this.setViewText(textCmp);
